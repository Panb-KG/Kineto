 ...
 *     GET /jobs/{id}/pose_data.json、GET /jobs/{id}/demo_output.mp4、GET /health、
 *     GET /healthz；其余一律 404（防止代理沦为任意 URL 中继）；
...
function isAllowedRoute(method: string, path: string[]): boolean {
  ...
  if (method === "GET") {
    ...
    if (
      p.length === 3 &&
      p[0] === "jobs" &&
      (p[2] === "pose_data.json" || p[2] === "demo_output.mp4")
    ) {
      return true; // /jobs/{id}/{pose_data.json|demo_output.mp4}
    }
  }
  return false;
}
